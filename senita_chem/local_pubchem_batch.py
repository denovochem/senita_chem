...
def batch_lookup_by_inchikeys_dict(
    inchikeys: List[str], local_pubchem_dict: Dict[str, Any]
) -> Dict[str, Dict]:
    """
    Look up InChIKeys in a local PubChem dictionary.

    Args:
        inchikeys: List of InChIKeys to look up
        local_pubchem_dict: Dictionary with InChIKeys as keys and PubChem data as values

    Returns:
        Dictionary with InChIKeys as keys and PubChem data as values
    """
    results = {}
    for inchi_key in inchikeys:
        if inchi_key in local_pubchem_dict:
            results[inchi_key] = local_pubchem_dict[inchi_key]
    return results
    ...
    try:
        ...
        for inchikey, synonym_text in synonym_rows:
            if inchikey in results:
                results[inchikey]["synonyms"].append(synonym_text)
...
if __name__ == "__main__":
    ...
    SMILES = [
        ...
        "C=C[C@](C)(O)CC[C@H]1C(C)=CC(=O)[C@H]2C(C)(C)CCC[C@]12C",
        ...
        "O=C1[C@@H]2[C@H](C(=O)N1CN(C(=O)C(F)(F)F)c1cccc(C(F)(F)F)c1)[C@H]1C=C[C@@H]2C1",
        "CCNc1ncc(COc2cccc3ccccc23)s1",
        "Cc1noc(C)c1CCCNC(=O)c1c(C)nn(Cc2ccccc2)c1C",
        "C[C@H]([NH2+]C1C[C@H](C)O[C@@H](C)C1)c1c[nH]c2cc(F)ccc12",
        ...
        "C/C=C/C=C/C(=O)N1C[C@H](C)[C@H](C(=O)OC)C1",
        ...
        "CC1(C)[C@H](NC(=O)CCNC(=O)C23CC4CC(CC(C4)C2)C3)[C@H]2CCO[C@@H]21",
        ...
        "C[C@H]1[C@@H](NC(=O)Cc2c[nH]c3ccccc23)CCC[C@@H]1C",
        ...
        "CC1(C)O[C@H]2O[C@H]([C@H]3COC(C)(C)O3)[C@H](OS(C)(=O)=O)[C@H]2O1",
        "Cc1cccc([C@H](CCl)CCC[C@@H]2CCCO2)c1",
    ]

    INCHI = [Chem.MolToInchiKey(Chem.MolFromSmiles(smi)) for smi in SMILES]
    ...
    results = batch_lookup_by_inchikeys_sqlite(
        inchikeys,
        "/home/csnbritt/projects/denovochem_projects/local_synonym_cas_project/pubchem.db",
    )
